...
public class KomponentaDAOImpl implements KomponentaDAO {
	...
	public boolean existByTip(String tip) throws SQLException {
			try (Connection connection = ConnectionUtil_HikariCP.getConnection()) {
				return existsByNazivTransactional(connection, tip);
			}
		}

		
	private boolean existsByNazivTransactional(Connection connection, String tip) throws SQLException {
		String query = "select * from komponenta where tip=?";
		try (PreparedStatement preparedStatement = connection.prepareStatement(query);) {
